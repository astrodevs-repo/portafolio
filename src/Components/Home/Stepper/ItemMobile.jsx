...
const ItemMobile = React.memo(function ItemMobile({ name, index, i, onObserver }) {
  const sections = [
    {
  ...
  return (
    <article
      className={`relative flex justify-center ${
        index < sections.length - 1 ? "divide-x-4 divide-blackCeniza dark:divide-gray-400" : ""
      }`}
      ref={ref}
    >
      <section>
      ...
      <Content currentStep={0} i={i} index={index} name={name} sections={sections} />
    </article>
  );
});
