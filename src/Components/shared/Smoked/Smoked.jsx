...
const Background = () => {
  ...
  const getRandomTransform = () => {
    const size = Math.random() * 150 + 350; // Tamaño entre 50 y 250 px
  ...
  const elements = Array.from({ length: 30 }).map((_, i) => (
    <motion.div
      ...
      transition={{
        repeat: Infinity,
        duration: 100, // Cambia la duración según la velocidad que desees
        ease: "linear",
      }}
    />
  ));
  ...
  return (
    <div className="absolute top-0 left-0 z-0 w-full h-screen overflow-hidden opacity-20">
      {elements}
    </div>
  );
};
